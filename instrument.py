...
class Instrument():
    ...
    def __init__(self, name, **kwargs):
        ...
        if self._lock_class in Instrument._lock_classes:
            ...
            self._access_lock = lib.calltimer.TimedLock(2.0)
    ...
    def has_tag(self, tags):
        ...
        if isinstance(tags, list): #type(tags) is types.ListType:
            for tag in tags:
    ...
    def add_parameter(self, name, **kwargs):
        ...
        if 'channels' in options:
            if len(options['channels']) == 2 and isinstance(options['channels'][0], int): # is types.IntType:
    ...
    def get_shared_parameter_options(self, name):
        ...
        if name in self._parameters:
            ...
            for i in ('get_func', 'set_func'):
                if i in options:
                    del options[i]
            if 'type' in options and isinstance(options['type'], type(None)): # is types.NoneType:
                options['type'] = None
            return options
        else:
            return None
    ...
    def format_parameter_value(self, param, val):
        ...
        try:
            if 'format_function' in opt:
                ...
                if isinstance(val, (list, tuple)): # in (types.ListType, types.TupleType):
                    val = tuple(val)

                elif isinstance(val, dict): # type(val) is types.DictType:
                    fmt = ""
                    first = True
                    for k in val.keys():
                        if first:
...
#                logging.debug('Trying to access cached value, but none available')
        ...
        if 'type' in p and value is not None:
            try:
                if isinstance(p['type'], int): #p['type'] == types.IntType:
                    value = int(value)
                elif isinstance(p['type'], float): #p['type'] == types.FloatType:
                    value = float(value)
                elif isinstance(p['type'], str): #p['type'] == types.StringType:
                    pass
                elif isinstance(p['type'], bool): #p['type'] == types.BooleanType:
                    value = bool(value)
                elif isinstance(p['type'], type(None)): #p['type'] == types.NoneType:
                    pass
    ...
    def get(self, name, query=True, fast=False, **kwargs):
        ...
        if isinstance(name, (list, tuple)): #type(name) in (types.ListType, types.TupleType):
            changed = {}
            result = {}
            for key in name:
    ...
    def _val_from_option_list(self, opts, value):
        if type(opts[0]) is not type(value):
            return None
        if isinstance(value, str): #type(value) is types.StringType:
            value = value.upper()

        match = None
        matches = 0
        for val in opts:
            if isinstance(val, str): #type(val) is types.StringType:
                val = val.upper()
    ...
    def _convert_value(self, value, ttype):
        if isinstance(value, bool) and not issubclass(ttype, bool): #type(value) is types.BooleanType and \
                #ttype is not types.BooleanType:
            logging.warning('Setting a boolean, but that is not the expected type')
            raise ValueError()

        if not isinstance(value, (int, float, str, bool, tuple, list)): #ttype not in self._CONVERT_MAP:
    ...
    def set(self, name, value=None, fast=False, **kwargs):
        ...
        result = True
        changed = {}
        if isinstance(name, dict): #type(name) == types.DictType:
